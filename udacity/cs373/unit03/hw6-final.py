...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if beta >= 0.001:
            ...
            xp = self.x + (distance * cos(self.orientation))
            yp = self.y + (distance * sin(self.orientation))
    ...
    # according to
    #           self.bearing_noise
    def sense(self, include_noise=1):
        Z = []

        for lmy, lmx in landmarks:
            dy = lmy - self.y
            dx = lmx - self.x
            noise = random.gauss(0, self.bearing_noise) if include_noise == 1 else 0.0
            new_o = atan2(dy, dx) - self.orientation + noise
            Z.append(new_o % (2 * pi))
